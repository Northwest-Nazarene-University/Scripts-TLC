...
scriptPurpose = r"""
This script (Get_Courses) uses the Canvas API to run a courses report and download the .csv result.
...
## with the Canvas Reports API and creates a .csv file of the courses in those Canvas school courses
def createCoursesCSV(p1_header, p1_inputTerm, attempt = 0):
    functionName = "createCoursesCSV"
    try:
        logger.info (f"     \n\nStarting {p1_inputTerm} Courses report")
        ...
        targetDestination = f"{termOutputPath}{p1_inputTerm}_Canvas_Courses.csv"
        ...
        ##Define and initialize the lists of relavent courses, the IDs of their related reports, and their download urls
        term_report_ID = ""
        ...
        logger.info ("\nChecking statuses of provisioning courses reports")
        ...
        while (status_text_jsonObject["progress"] != 100):
            ## Wait 10 seconds
            logger.info ("\n" + f"{p1_inputTerm} Courses report is incomplete. Waiting 10 seconds")
            time.sleep(10)
        ...
        if "parameters" in status_text_jsonObject.keys():
            if "extra_text" in status_text_jsonObject["parameters"].keys():
                if "failed" in status_text_jsonObject["parameters"]["extra_text"]:
                    ...
                    ## Recursively call this function again unless this is the third attempt
                    if attempt != 3:
                        createCoursesCSV(p1_header, p1_inputTerm, attempt + 1)
                    return targetDestination
        ...
        logger.info ("\nDownloading provisioning courses report")
        downloadFile(term_report_download_url, targetDestination, "w")

        ## Log that the file was downloaded
        logger.info (f"     \nSucessfully downloaded {p1_inputTerm} Course CSV")

        ## Sleep 5 seconds to ensure the file is done writing
        time.sleep(5)
...
def termGetCourses(inputTerm = "All"):
    functionName = "Get_Courses"
    ...
    resultFilePath = createCoursesCSV(p1_header = header, p1_inputTerm = inputTerm)
...
if __name__ == "__main__":
    ...
    ## Start and download the Canvas report
    createCoursesCSV (p1_header = header, p1_inputTerm = input("Enter the desired term in \
