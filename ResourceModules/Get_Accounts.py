...
scriptName = "Get_Accounts"

scriptPurpose = r"""
This script (Get_Accounts) uses the Canvas API to run a accounts report and download the .csv result.
It requires the following paramters: the path the .csv should be saved to (p1_SavePath), the API 
header (p1_header), and the .
"""
...
## This function uses the Account provided term to determines which Canvas school accounts to pull accounts from
## with the Canvas Reports API and creates a .csv file of the accounts in those Canvas school accounts
def createAccountsCSV(p1_header, attempt = 0):
    ...
    try:
        ...
        report_ID = report_text_jsonObject["id"]

        ## Check the status of the report to see if it has finished
        ...
        try: ## Irregular try clause, do not comment out in testing
            ...
            logger.warning (f"\nError: No download url for the  report")
