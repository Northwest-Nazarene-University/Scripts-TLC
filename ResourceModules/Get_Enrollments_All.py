...
scriptName = "Get_Enrollments_All"

scriptPurpose = r"""
This script (Get_Enrollments_All) uses the Canvas API to run a enrollments report and download the .csv result.
...
def createEnrollmentsCSV(p1_header, p1_inputTerm, attempt = 0):
    functionName = "createEnrollmentsCSV"
    try:
        logger.info (f"     \n\nStarting {p1_inputTerm} Enrollment report")
        ...
        if p1_inputTerm:
            targetDestination = f"{termOutputPath}{p1_inputTerm}_Complete_Canvas_Enrollments.csv"
        else:
            targetDestination = f"{outputPath}Complete_Canvas_Enrollments.csv"
        ...
        ## Make an api call to start a provisioning report for each of the relavent enrollments and append the report id to the term_report_ID list
        logger.info ("\n" + "Calling provisioning enrollments reports")

        ## Initilize payload with one of the relavent enrollments
        payload = None
        if p1_inputTerm:
            payload = {'parameters[include_deleted]':'true', 'parameters[enrollments]':'true', "parameters[enrollment_term_id]":f"sis_term_id:{p1_inputTerm}"}
        else:
            payload = {'parameters[enrollments]':'true', 'parameters[include_deleted]':'true'}
        ...
        while (status_text_jsonObject["progress"] != 100):
            ## Wait 10 seconds
            logger.info ("\n" + f"{p1_inputTerm} Enrollment report is incomplete. Waiting 10 seconds")
            time.sleep(10)
        ...
        if "parameters" in status_text_jsonObject.keys():
            if "extra_text" in status_text_jsonObject["parameters"].keys():
                if "failed" in status_text_jsonObject["parameters"]["extra_text"]:
                    ...
                    ## Recursively call this function fagain unless this is the fifth attempt
                    if attempt != 3:
                        createEnrollmentsCSV(p1_header, p1_inputTerm, attempt + 1)
                    return
        ...
        logger.info (f"\nDownloading {p1_inputTerm} Enrollment CSV")
        if p1_inputTerm:
            downloadFile(term_report_download_url, f"{termOutputPath}{p1_inputTerm}_Complete_Canvas_Enrollments.csv", "w")
        else:
            downloadFile(term_report_download_url, f"{outputPath}_Complete_Canvas_Enrollments.csv", "w")

        logger.info (f"     \nSucessfully downloaded {p1_inputTerm} Enrollment CSV")

    except Exception as error:
        error_handler (functionName, p1_ErrorInfo = error)

def termGetEnrollments(inputTerm = ""):
    functionName = "Get_Enrollments_All"
    ...
    createEnrollmentsCSV(p1_header = header, p1_inputTerm = inputTerm)
...
if __name__ == "__main__":
    ...
    ## Start and download the Canvas report
    createEnrollmentsCSV (p1_header = header, p1_inputTerm = input("Enter the desired term in \
