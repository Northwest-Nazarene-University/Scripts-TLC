...
import traceback, requests, time, json, os, logging, sys, re

# Define the script name, purpose, and external requirements for logging and error reporting purposes
scriptName = "Get_Enrollments"

scriptPurpose = r"""
This script (Get_Enrollments) uses the Canvas API to run a enrollments report and download the .csv result.
...
def createEnrollmentsCSV(p1_header, p1_inputTerm, attempt = 0):
    ...
    try:
        ...
        if p1_inputTerm:
            targetDestination = f"{termOutputPath}{p1_inputTerm}_Canvas_Enrollments.csv"
        else:
            targetDestination = f"{outputPath}Canvas_Enrollments.csv"
        ...
        payload = {'parameters[enrollments]':'true'
                   , 'parameters[include_deleted]':'true'
                   } if p1_inputTerm == "All" \
            else {'parameters[include_deleted]':'true'
                  , 'parameters[enrollments]':'true'
                  , 'parameters[created_by_sis]':'true'
                  , "parameters[enrollment_term_id]":f"sis_term_id:{p1_inputTerm}"}
        ...
        ## Wait 5 seconds to ensure that the file has finished writing
        time.sleep(5)
...
def termGetEnrollments(inputTerm = "All"):
    functionName = "Get_Enrollments"
    ...
    targetDestination = createEnrollmentsCSV(p1_header = header, p1_inputTerm = inputTerm)
