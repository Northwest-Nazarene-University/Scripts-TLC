...
def createTugStudentsCsv(p1_inputTerm, attempt = 0):
    functionName = "createUsersCSV"
    try:
        logger.info (f"     \nStarting TUG Student report")
        ...
        targetDestination = f"{termOutputPath}{p1_inputTerm}_TUG_Students.csv"
        ...
        rawCanvasCourses["account_id"] = rawCanvasCourses["account_id"].fillna("")

        ## Filter the sis courses to only include TUG courses by exluding all rows that have "G_" or "_APP"
        ## in the account_id column and have TRUE in the created_by_sis column
        tugCanvasCourses = rawCanvasCourses[
            ~rawCanvasCourses["account_id"].str.contains("G_|_APP")
        ...
        ## Filter the sis enrollments to only include those with that are enrolled in a tugSisCourse course that have created by_sis set to TRUE
        tugCanvasEnrollments = rawCanvasEnrollments[rawCanvasEnrollments["canvas_course_id"].isin(tugCanvasCourses["canvas_course_id"])
                                                    & (rawCanvasEnrollments["created_by_sis"] == True)
                                                    & (rawCanvasEnrollments["role"] == "student")
                                           ]

        ## Read (and update if neccessary) the canvas users csv fil6e for the term as a pandas df
        rawCanvasUsers = pd.read_csv(getUsers(inputTerm = p1_inputTerm))

        ## Filter the sis users to only include those whose ids are in the unique user_id column of the tugCanvasEnrollments df
        tugCanvasUsers = rawCanvasUsers[rawCanvasUsers["canvas_user_id"].isin(tugCanvasEnrollments["canvas_user_id"].unique())]

        ## Save the filtered sis users to a csv file in the outputPath
        tugCanvasUsers.to_csv(targetDestination, index=False)

        logger.info (f"     \nTUG Student report created successfully at {outputPath}TUG_Students.csv")
...
def termGetTugStudents(inputTerm = ""):
    functionName = "Get_TUG_Students"

    ## Start and download the Canvas report
    targetDestiniation = createTugStudentsCsv(p1_inputTerm = inputTerm)
...
if __name__ == "__main__":
    ...
    ## Start the tug student report creation process
    termGetTugStudents (inputTerm = input("Enter the desired term in \
