...
scriptName = "Get_Unpublished_Courses"

scriptPurpose = r"""
This script (Get_Unpublished_Courses) uses the Canvas API to run a unpublished courses report and download the .csv result.
...
## This function uses the user provided term to determines which Canvas school unpublished courses to pull users from
## with the Canvas Reports API and creates a .csv file of the courses in those Canvas school unpublished courses
def createUnpublished_CoursesCSV(p1_header, p1_inputTerm, attempt = 0):
    functionName = "createUnpublished_CoursesCSV"
    try:
        logger.info (f"     \n\nStarting {p1_inputTerm} Unpublished_Courses report")
        ...
        targetDestination = f"{termOutputPath}{p1_inputTerm}_Canvas_Unpublished_Courses.csv"
        ...
        start_report_API_URL = CoreCanvasAPIUrl + "accounts/1/reports/unpublished_courses_csv"
        
        ##Define and initialize the lists of relavent unpublished courses, the IDs of their related reports, and their download urls
        term_report_ID = ""
        term_report_download_url = ""
        
        ## Make an api call to start a provisioning report for each of the relavent unpublished courses and append the report id to the term_report_ID list
        logger.info ("\n" + "Calling provisioning unpublished courses reports")
        ...
        status_report_API_URL = CoreCanvasAPIUrl + "accounts/1/reports/unpublished_courses_csv/" + str(term_report_ID)
        ...
        while (status_text_jsonObject["progress"] != 100):
            ## Wait 10 seconds
            logger.info ("\n" + f"{p1_inputTerm} Unpublished_Courses report is incomplete. Waiting 10 seconds")
            time.sleep(10)
        ...
        if "parameters" in status_text_jsonObject.keys():
            if "extra_text" in status_text_jsonObject["parameters"].keys():
                if "failed" in status_text_jsonObject["parameters"]["extra_text"]:
                    ...
                    if attempt != 3:
                        createUnpublished_CoursesCSV(p1_header, p1_inputTerm, attempt + 1)
                    return
        ...
        logger.info (F"\nDownloading {p1_inputTerm} Unpublished_Courses CSV")
        downloadFile(term_report_download_url, f"{termOutputPath}{p1_inputTerm}_Canvas_Unpublished_Courses.csv", "w")

        logger.info (f"     \nSucessfully downloaded {p1_inputTerm} Unpublished_Courses CSV")

    except Exception as error:
        error_handler (functionName, p1_ErrorInfo = error)

def termGetUnpublishedCourses(inputTerm):
    functionName = "Get_Unpublished_Courses"
    ...
    createUnpublished_CoursesCSV(p1_header = header, p1_inputTerm = inputTerm)
...
if __name__ == "__main__":
    ...
    ## Start and download the Canvas report
    createUnpublished_CoursesCSV (p1_header = header, p1_inputTerm = input("Enter the desired term in \
