...
import os, sys, logging, os, os.path
import pandas as pd
...
def determineDepartmentSavePath(courseAccountId):
    ...
    try:
        ...
        ## If the courseAccountId == 1
        if courseAccountId == 1:

            ## Return the root account path
            return "NNU\\"
        ...
        ## Set the targetRow to account_id
        targetRow = "account_id"
        
        ## If the courseAccountId is not in the departmentSavePathsDf's account_id column
        if courseAccountId not in departmentSavePathsDF[targetRow].tolist():

            ## Set the targetRow to canvas_account_id
        ...
        if len(accountSavePath.rsplit("\\")) > 3:
            ...
            accountSavePath = accountSavePath\
                .replace(f"Undergraduate {collegeName}", "Undergraduate")\
                .replace(f"Graduate {collegeName}", "Graduate")
